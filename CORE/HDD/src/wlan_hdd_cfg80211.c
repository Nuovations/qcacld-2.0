                                           ...
                                           int data_len)
{
    ...
    nla_for_each_nested(buckets,
        ...
        if (nla_parse(bucket,
            QCA_WLAN_VENDOR_ATTR_EXTSCAN_SUBCMD_CONFIG_PARAM_MAX,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,11,0))
            nla_data(buckets), nla_len(buckets), NULL, NULL)) {
#else
...
int wlan_hdd_cfg80211_update_band(struct wiphy *wiphy, eCsrBand eBand)
{
    ...
    for (i = 0; i < NUM_NL80211_BANDS; i++)
    {
        ...
        for (j = 0; j < wiphy->bands[i]->n_channels; j++)
        {
            ...
            if (NL80211_BAND_2GHZ == i && eCSR_BAND_5G == eBand) // 5G only
            {
            ...
            else if (NL80211_BAND_5GHZ == i && eCSR_BAND_24 == eBand) // 2G only
            {
                band->channels[j].flags |= IEEE80211_CHAN_DISABLED;
                continue;
            }
...
#else
    sinfo->filled |= BIT(NL80211_STA_INFO_TX_PACKETS) | BIT(NL80211_STA_INFO_TX_RETRIES) | BIT(NL80211_STA_INFO_TX_FAILED);
#endif
